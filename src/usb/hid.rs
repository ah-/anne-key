...
fn usb_clear_tx_ep1_ctr(r: &mut super::super::USB_LP::Resources) {
    r.USB.usb.usb_ep1r.write(|w| unsafe {
        w.bits(
            (r.USB.usb.usb_ep1r.read().bits() & 0xFF7F) & super::USB_EPREG_MASK,
        )
    });
}
...
fn usb_clear_rx_ep1_ctr(r: &mut super::super::USB_LP::Resources) {
    r.USB.usb.usb_ep1r.write(|w| unsafe {
        w.bits(
            (r.USB.usb.usb_ep1r.read().bits() & 0x7FFF) & super::USB_EPREG_MASK,
        )
    });
}
...
fn set_ep1_tx_status_valid_dtog(r: &mut super::super::USB_LP::Resources) {
    let mut bb = r.USB.usb.usb_ep1r.read().bits();
